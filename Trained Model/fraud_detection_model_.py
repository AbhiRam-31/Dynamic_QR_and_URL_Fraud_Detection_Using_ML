...
class HybridFraudDetector:
    ...
    def _extract_link_features(self, url, offline_mode=False):
        """
        ...
        try:
            ...
            if domain and '.' in domain and self.has_whois and not offline_mode:
                 try:
            ...
            # ---Dynamic HTTP Features (Network Requests) ---
            num_redirects = 0
            is_final_domain_different = 0
            is_https_valid = 0
            http_response_code = -1 # Indicates failure or timeout

            if not offline_mode:
                try:
                    # Perform a GET request to the URL with a shorter timeout
                    # Allow redirects to count them and get the final URL
                    import requests
                    ...
                    # Compare final domain with original domain
                    try:
                        final_domain = urllib.parse.urlparse(final_url).netloc
                        if '@' in final_domain:
                            final_domain = final_domain.split('@')[-1]
                        if ':' in final_domain:
                            final_domain = final_domain.split(':')[0]
                        if final_domain.startswith('www.'):
                            final_domain = final_domain[4:]
                            
                        # Compare cleaned domains
                        is_final_domain_different = int(final_domain != domain and final_domain != "")
                    except Exception:
                        is_final_domain_different = 0
                        
                    # Check HTTPS validation. If requests succeeded without throwing SSLError, it's valid (if it's https)
                    ...
                    # Generic network failure (timeout, connection refused, offline)
                    logger.debug(f"Request failed for {url}: {e}")
            else:
                # Offline mode (like during synthetic training)
                if has_https:
                    is_https_valid = 1
                http_response_code = 200 # Assume success for synthetic training to not penalize
            # --- END ADDED ---
    ...
    def train_model(self, n_samples=10000, data_path=None):
        ...
        try:
            ...
            logger.info("Extracting features for the training dataset (in offline mode to prevent hanging)...")
            # Use the _extract_link_features method for consistency. It handles its own errors and returns zeros on critical failure.
            # List comprehension to extract features for each URL in offline_mode.
            extracted_features_list = [self._extract_link_features(url, offline_mode=True) for url in urls]
    ...
    def analyze_url(self, url):
        ...
        try:
            ...
            if features.shape[0] > 0 and features.shape[1] == EXPECTED_FEATURE_COUNT:
                ...
                num_redirects_val = extracted_features_dict.get("num_redirects", 0)
                if num_redirects_val > 2:
                    risk_factors.append(f"Multiple URL redirects detected ({int(num_redirects_val)} redirects)")
                    
                if extracted_features_dict.get("is_final_domain_different", 0) == 1:
                    risk_factors.append("URL forcefully redirects to a completely different domain")
                    
                http_response_val = extracted_features_dict.get("http_response_code", 200)
                if http_response_val != 200 and http_response_val != -1: # It reached something but had an error
                    risk_factors.append(f"Server returned HTTP error {int(http_response_val)}")
                elif http_response_val == -1:
                    risk_factors.append("Website is currently offline or unreachable")
                    
                has_https_flag = extracted_features_dict.get("has_https", 0)
                if has_https_flag == 1 and extracted_features_dict.get("is_https_valid", 0) == 0:
                    risk_factors.append("HTTPS SSL Certificate is missing or invalid")
