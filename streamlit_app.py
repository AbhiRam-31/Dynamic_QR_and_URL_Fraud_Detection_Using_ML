...
st.set_page_config(
    page_title="Unified QR and URL-Based Phishing Detection framework", # Title displayed in the browser tab
...
def analyze_url_api(url):
    ...
    if analysis_response_data: # Data is None if call_backend_api displayed an error
        ...
        # Intentionally NOT calling st.rerun() here so the error message remains visible

    if analysis_response_data:
        st.rerun() # Trigger a rerun to update the UI and display the analysis result
...
def analyze_qr_api(uploaded_file):
    ...
    if st.session_state.get('last_analysis_result'):
        st.rerun() # Trigger a rerun to update the UI and display the results
...
st.title("Unified QR and URL Phishing Detection Framework")
